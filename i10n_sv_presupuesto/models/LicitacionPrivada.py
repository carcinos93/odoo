...
class LicitacionPrivada(models.Model):
    _name = 'presupuesto.licitacion_privada'
    ...
    documentos = fields.One2many(comodel_name='presupuesto.documentos_licitacion_privada', inverse_name='licitacionPrivada_ids', string='Documentos')


class DocumentosLicitacionPrivada(models.Model):
    _name = 'presupuesto.documentos_licitacion_privada'
    codigoTipoDocumento = fields.Char(string='Código tipo documento', required=False)
    archivo = fields.Binary(string="Carga de archivo" )
    licitacionPrivada_ids = fields.Many2one(comodel_name='presupuesto.licitacion_privada', string='Licitación privada')
