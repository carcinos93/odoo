...
class LicitacionPublica(models.Model):
    _name = 'presupuesto.licitacion_publica'
    ...
    documentosBases = fields.One2many(comodel_name='presupuesto.documentos_bases_licitacion_publica', inverse_name='licitacionPublica_ids', string='Documentos bases')
    documentosParticipantes = fields.One2many(comodel_name='presupuesto.documentos_participantes_licitacion_publica', inverse_name='licitacionPublica_ids', string='Documentos participantes')
    documentosAdjudicaciones = fields.One2many(comodel_name='presupuesto.documentos_adjudicacion_licitacion_publica', inverse_name='licitacionPublica_ids', string='Documentos adjucicación')


class DocumentosBasesLicitacion(models.Model):
    _name = 'presupuesto.documentos_bases_licitacion_publica'
    ...
    licitacionPublica_ids = fields.Many2one(comodel_name='presupuesto.licitacion_publica', string='Licitación pública')


class DocumentosParticipantesLicitacion(models.Model):
    _name = 'presupuesto.documentos_participantes_licitacion_publica'
    codigoTipoDocumento = fields.Selection(string='Código tipo documento', selection=[])
    archivo = fields.Binary(string="Carga de archivo")
    palabrasClave = fields.Char(string='Palabras clave', required=False)
    licitacionPublica_ids = fields.Many2one(comodel_name='presupuesto.licitacion_publica', string='Licitación pública')
...
class DocumentosParticipantesLicitacion(models.Model):
    _name = 'presupuesto.documentos_adjudicacion_licitacion_publica'
    codigoTipoDocumento = fields.Selection(string='Código tipo documento', selection=[])
    archivo = fields.Binary(string="Carga de archivo")
    palabrasClave = fields.Char(string='Palabras clave', required=False)
    licitacionPublica_ids = fields.Many2one(comodel_name='presupuesto.licitacion_publica',
